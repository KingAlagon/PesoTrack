          ...
          : provider.goals.isEmpty
              ...
              : ListView(
                  ...
                  children: [
                    ...
                    ...provider.goals.map((g) => _GoalCard(goal: g, provider: provider)),
                  ],
                ),
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => Navigator.push(
            context, MaterialPageRoute(builder: (_) => const AddSavingsGoalScreen())),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Text('${provider.completedCount}/${provider.goals.length} goals done',
                style: const TextStyle(color: Colors.white70, fontSize: 13)),
          ]),
          const SizedBox(height: 8),
          ClipRRect(
          ...
          children: [
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('₱ ${fmt.format(goal.currentAmount)}',
                  style: TextStyle(color: goal.color, fontWeight: FontWeight.bold, fontSize: 18)),
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              if (goal.deadline != null)
                Text('Due ${DateFormat('MMM d, y').format(goal.deadline!)}',
                    style: const TextStyle(color: AppColors.textSecondary, fontSize: 12)),
            ]),
          ],
        ),
    ...
    switch (action) {
      ...
      case 'edit':
        Navigator.push(context,
            MaterialPageRoute(builder: (_) => AddSavingsGoalScreen(goal: goal)));
      case 'delete':
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
          TextButton(
            onPressed: () { provider.delete(goal.id); Navigator.pop(ctx); },
  ...
  Widget build(BuildContext context) {
    return const Center(
      child: Column(
        ...
        children: [
          Icon(Icons.savings, size: 64, color: AppColors.textSecondary),
          SizedBox(height: 16),
          Text('No savings goals yet', style: TextStyle(fontSize: 18, color: AppColors.textSecondary)),
          Text('Tap + to create your first goal', style: TextStyle(color: AppColors.textSecondary)),
        ],
      ),
    );
  }
